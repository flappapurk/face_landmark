...
class YOLOv8_face:
    ...
    def draw_detections(self, image, boxes, scores, kpts):
        ...
        for box, score, kp in zip(boxes, scores, kpts):
            ...
            cv2.rectangle(image, (x, y), (x + w, y + h),
                          (0, 200, 255), thickness=3)
            cv2.putText(image, f"face-{num}:" + str(round(score, 2)), (x, y - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), thickness=3)

            # key features [eye-l, eye-r, nose, mouth-r,mouth-l]
            features_idx = [(0, 1), (3, 4),  (12, 13), (9, 10)]
            # features_idx = [(0, 1), (3, 4), (6, 7), (12, 13), (9, 10)]
            for i in features_idx:
                ix, iy = i

                cv2.circle(
                    image, (int(kp[ix]), int(kp[iy])), 5, (0, 255, 0), thickness=-1)
    ...
    def face_features_visible(self, boxes,  kpts, threshold=0.1585):
        results=[]
        for box, kp in zip(boxes, kpts):
            ...
            for i in features_idx:
                ix, iy = i
                keypoints.append((int(kp[ix]), int(kp[iy])))
            # print(keypoints)
            face_area_s = polygon_area(keypoints)
            face_area_l = w * h
            results.append ( (face_area_s/face_area_l) > threshold)
